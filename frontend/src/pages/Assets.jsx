...
export default function Assets() {
  ...
  return <div className="secondary-page">
    ...
    <div className="asset-cards">{catalog.assets.map(a => <article className="asset-card" key={a.symbol} style={{ '--asset-color': a.color }} data-testid={`asset-card-${a.symbol}`}>
      ...
      <div className="asset-card-stats"><div><span>REWARD RANGE</span><strong data-testid={`asset-range-${a.symbol}`}>{money(a.min_reward)} — {money(a.max_reward)}</strong></div><div><span>DROP CHANCE</span><strong className="asset-color" data-testid={`asset-odds-${a.symbol}`}>{a.odds}%</strong></div></div>
    ...
    <section className="pool-method">
      <div><div className="eyebrow">THE NUMBERS, WITHOUT THE MYSTERY</div><h2>One pool. Equal access.</h2><p data-testid="reward-pool-description">Every pod series uses the same reward pool. Your INC amount and pod appearance do not change the asset probabilities or reward ranges.</p><Link to="/zone" className="primary-link" data-testid="assets-to-zone">Find your incubation pod <ArrowUpRight size={16}/></Link></div>
      <div className="method-items">
        ...
        <div><Shuffle size={20}/><span><b>Every cycle is independent</b><p>Your previous rewards do not change the next result.</p></span></div>
      </div>
    </section>
  </div>;
}