...
export default function Zone({ onStart, onReveal, onSettings }) {
  ...
  const skip = async () => { if (await action('fast-forward')) toast.success('Cycle complete. RWA allocations are ready to reveal.'); };
  return <>
    <section className="zone-hero"><div className="hero-grid"/><div className="hero-copy"><div className="hero-eyebrow" data-testid="zone-eyebrow"><span className="eyebrow-line"/>REAL-WORLD VALUE. INCUBATED.</div><h1 data-testid="zone-hero-title">THE INCUBATION<br/><span>ZONE.</span><span className="heading-period">↗</span></h1><p data-testid="zone-description">INC powers a shared cycle of possibility. Two hours.<br className="desktop-break"/> One random RWA reward. 75% returned. 25% burned.</p><div className="hero-bottom"><ChainBadge/><span className="hero-separator"/><span className="hero-version">A NEW CYCLE OF OWNERSHIP</span></div></div><div className="hero-object"><PodScene hero testId="hero-pod-canvas"/><div className="object-label label-top"><span className="tiny-dot"/>QUANTUM INCUBATOR <span>GEN. 01</span></div><div className="object-label label-right"><span>RWA CORE</span><b data-testid="hero-asset-count">5 RWA ASSETS</b><i/></div><div className="object-label label-bottom"><span>INC.HOOD / SYSTEM ONLINE</span><span>↗</span></div></div><div className="hero-corner">INCUBATION PROTOCOL <span>001 — 006</span></div></section>
    <MetricStrip/>
    <section className="zone-workspace"><div className="workspace-main"><div className="section-toolbar"><div className="section-title"><span className="section-square"/><h2>Incubation pods</h2><span className="count">06</span></div><div className="toolbar-actions"><button className="text-button skip-button" disabled={!active || busy} onClick={skip} data-testid="demo-fast-forward-button" title="Complete all active cycles"><FastForward size={13}/>Skip 2h</button><button className="icon-button" onClick={onSettings} title="Configure incubation amount" aria-label="Configure incubation amount" data-testid="zone-settings-button"><SlidersHorizontal size={16}/></button></div></div>
